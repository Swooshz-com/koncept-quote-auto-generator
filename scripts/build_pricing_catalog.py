...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rag-md-out", type=Path, help="Optional generated Markdown view for AI/RAG reading.")
...
def catalog_to_rag_markdown(catalog: dict[str, Any], catalog_name: str = "pricing-catalog.json") -> str:
    lines = [
        "---",
        "title: Pricing Catalog RAG View",
        "kind: generated_pricing_rag_view",
        f"source_catalog: {catalog_name}",
        "---",
        "",
        "# Pricing Catalog RAG View",
        "",
        "Generated from the structured pricing catalog. Do not edit this file directly.",
        "Use this Markdown for AI retrieval/readability; use the JSON catalog for pricing logic.",
        "",
    ]
...
def write_rag_markdown(catalog: dict[str, Any], out: Path, catalog_name: str = "pricing-catalog.json") -> None:
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(catalog_to_rag_markdown(catalog, catalog_name), encoding="utf-8")
...
def main() -> None:
    ...
    if args.rag_md_out:
        write_rag_markdown(catalog, args.rag_md_out, args.out.name)
        print(f"Wrote {args.rag_md_out}")
