...
- Never add pricing-reference semantic families, synonym packs, customer/sample aliases, catalog-specific object matching branches, or V1.1/Koncept-only matching helpers to source code. Pricing `match_terms` and `object_families` must come from saved pricing-reference data, AI import output, user-maintained data files, or generic token/phrase algorithms. Keep `scripts/validate_dynamic_pricing_reference_rules.py` and CI guarding this rule.
