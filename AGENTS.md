...
- Use `references/quotation-cost-template.md` as the only authoritative pricing source and `references/quotation-layout.xlsx` as the customer-facing quote layout source.
- Preserve the formatting rules in `references/quotation-format.md`, including readable quantity widths, bold table headers, centered quantity values, thousands separators, dynamic totals, and a header logo/details group that stays inside the print area.
